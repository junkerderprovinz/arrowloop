...
**A job can report instead of writing.** `reportOnly` runs the whole comparison on the schedule, writes down what it WOULD have done per file, notifies, and touches nothing. It is not the same as switching a job off: a disabled job tells you nothing, and the job somebody is nervous about is the one they most want a report on. Pressing the button by hand still writes, because pressing the button IS the decision this withholds from the clock.
...
**The look is [GlimStone](https://github.com/junkerderprovinz/glimstone) 1.7.4**, and its reference tokens and appearance engine are copied in verbatim rather than reimplemented. Theme, corner shape and accent are the viewer's to set. The accent marks activity and nothing else, which is why the switches down the preview are deliberately colourless: every row arrives ticked, and a control that is on in all of them is not activity.
