...
- **Files another program holds open are copied from a shadow copy** on Windows, when ArrowLoop runs with administrator rights, as `arrowloop service` does. It takes one shadow copy per volume and run, only once such a file turns up, and removes it when the run ends. Without those rights the file is postponed as before, and the reason says that the service could copy it. Renames, removals and conflicts still wait for the file to be closed.
- **Windows on ARM.** A computer with an ARM processor, such as a Snapdragon laptop, gets its own installer and portable exe instead of running the `amd64` build through emulation. The App tab in the settings offers both.
- **Commands before and after a run.** A job's `before` command runs before the two sides are compared, to write a database dump or stop a service, and the run does not start if it fails. Its `after` command runs once the run is over, learns from its environment how it went and what was copied, moved and deleted, and marks the run failed if it fails itself. Both can only be written in `arrowloop.json`: anybody who can reach the interface could otherwise run anything on the machine, so the job editor shows them and every save from the interface keeps what the file says. See [Configuring a job](docs/configuration.md#commands-before-and-after-a-run).
...
- **The phone app's overview uses notch cards**, one per section, instead of plain titles above loose cards, and every page title is set at the heading size. At Wild and Storm the jobs and the history arrive like the other tabs, and a page that is flung against its top or bottom runs on past the edge and springs back.
- **The phone app moves at every motion level.** Cards arrive as a page opens and again when its tab comes back, one after another; at Wild they fly in from both sides and bounce into place, at Storm further still. Buttons and cards give way under a finger and spring back. The overview says "Deleted locally" for a file removed on the phone.
...
- **A button's glyph is the size of its words**, 14px instead of 20px, so a row of buttons no longer reads as a row of heavy icons.
- **The history reaches the bottom of the window**, and the file log loads the next lines when its end scrolls into view instead of scrolling inside a box of its own. A run reads "Completed" or "Error", and the filter "Only errors".
- **Stacked selectors no longer repeat one colour down the page.** The three label rows, and the direction, mode and case rows on the engine page, each start one step further along the palette, the same as in BombVault.
- **A selector with four options stays on one line.** It counted the card's padding as room, so with the storm level showing, the fourth option wrapped under the other three. The settings column is also wide enough now for four options at full width. Where a window is too narrow and a selector wraps after all, its options share the rows evenly and fill them, two and two rather than three and one.
- **Adding a target opens a window over the dimmed page**, with the provider tiles first and the form after them. While only the tiles are showing, a click beside the window closes it; once the form is open, only Cancel, Close or Escape do, so a stray click cannot throw away what was typed.
- **Each target card has its own button and empty line**: Connect a cloud, Add object storage, Add a server or share. All three used to say "Add storage or cloud", and the servers card talked about S3 buckets.
